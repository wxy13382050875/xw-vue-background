  ...
  created() {
    if (typeof (this.$route.query.PersonId) !== 'undefined') {
      ...
      getPersonInfo(params).then(response => {
        ...
        if (response.data.PersonJobs.length > 0) {
          this.PersonJob = response.data.PersonJobs
        }
        if (response.data.PersonFamily.length > 0) {
          this.PersonFamily = response.data.PersonFamily
        }
